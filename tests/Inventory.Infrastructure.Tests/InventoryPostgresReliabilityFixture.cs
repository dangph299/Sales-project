...
/// <summary>
/// Provides a real PostgreSQL for the Inventory reliability suite. When Docker is unavailable
/// the fixture reports <see cref="IsAvailable"/> as <see langword="false"/> so tests skip visibly
/// instead of passing silently.
/// </summary>
/// <remarks>
/// A connection string in <c>INVENTORY_TEST_POSTGRES</c> is used in preference to starting a
/// container. The database it names is dropped and recreated by the migration tests, so it must be
/// a dedicated test database and never a development one.
/// </remarks>
public sealed class InventoryPostgresReliabilityFixture : IAsyncLifetime
{
    private const string ConnectionStringVariable = "INVENTORY_TEST_POSTGRES";
