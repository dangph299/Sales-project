...
public sealed class ConfirmOrderHandler(IOrderRepository orders, IUnitOfWork uow, ILogger<ConfirmOrderHandler> logger) : IRequestHandler<ConfirmOrder, OrderDto>
{
    ...
    /// </summary>
    /// <param name="request">
    /// The command identifying the order to confirm and its expected version.
    /// </param>
    /// <param name="ct">
    /// A token to observe while waiting for the operation to complete.
    /// </param>
    /// <returns>
    /// The order in its PendingInventory status, mapped to an <see cref="OrderDto"/>.
    /// </returns>
    /// <exception cref="NotFoundException">
    /// Thrown when no order exists with the given identifier.
    /// </exception>
    /// <exception cref="ConflictException">
    /// Thrown when the order's actual version does not match <see cref="ConfirmOrder.ExpectedVersion"/>.
    /// </exception>
    /// <exception cref="Sales.Domain.DomainException">
    /// Thrown when the order is not in the Draft status.
    /// </exception>
