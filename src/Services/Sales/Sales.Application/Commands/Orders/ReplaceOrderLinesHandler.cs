    ...
    : IRequestHandler<ReplaceOrderLines, OrderDto>
{
    ...
    /// </summary>
    /// <param name="request">
    /// The command identifying the order, its expected version, and the new lines.
    /// </param>
    /// <param name="ct">
    /// A token to observe while waiting for the operation to complete.
    /// </param>
    /// <returns>
    /// The order with its new lines, mapped to an <see cref="OrderDto"/>.
    /// </returns>
    /// <exception cref="NotFoundException">
    /// Thrown when the order or one of the requested products does not exist.
    /// </exception>
    /// <exception cref="ConflictException">
    /// Thrown when the order's actual version does not match <see cref="ReplaceOrderLines.ExpectedVersion"/>.
    /// </exception>
    /// <exception cref="Sales.Domain.DomainException">
    /// Thrown when the order is not in the Draft status, a requested product is inactive, or the
    /// requested lines are empty or contain a repeated product.
    /// </exception>
