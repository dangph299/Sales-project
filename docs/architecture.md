...
- `Common/Exceptions`: application-level orchestration errors, not domain rules (`NotFoundException`, `ConflictException`). Their HTTP status, public error code, and log severity are decided at the web boundary by `ConfigureSalesExceptions` in `Sales.Api/Extensions/ServiceCollectionExtensions.cs`, not in the Application layer.
- `Validators`: request validation before a command reaches the aggregate. Domain invariants must still be enforced in Domain.

Note: the MediatR pipeline behaviors (`LoggingBehavior`, `PerformanceBehavior`, `ValidationBehavior`) live in `BuildingBlocks.Application/Behaviors` and are registered via `AddApplicationBuildingBlocks()`, called from `Sales.Application`'s own `AddSalesApplication()` — see `BuildingBlocks.Application` below. None of them log failures above Debug: each execution path logs its own failures once at its own boundary (`ApiExceptionHandler` for HTTP, `IntegrationEventHandler` for Kafka, the outbox/inbox services for their cycles, Hangfire for jobs).
...
- `Inventory.Infrastructure/Persistence`: Inventory database, Inbox, and migrations. The Outbox and Inbox entities themselves live in `BuildingBlocks.Infrastructure` (`InboxMessage` shared with Sales, each service owning its own EF mapping — see below); only the service-specific access code (Inventory's `IInventoryInbox` port) stays local.
...
- `AuditLog.Infrastructure/Mongo`: MongoDB document model, repository behavior, and unique `AuditId` handling for `AuditLogEvent` documents.
...
- `BuildingBlocks.Application`: shared Application-layer building blocks referenced by Sales and Inventory Application layers — `Behaviors/LoggingBehavior`, `Behaviors/PerformanceBehavior`, `Behaviors/ValidationBehavior`, `Persistence/IUnitOfWork`, `Pagination/PagedResult`/`Pagination/Paging`, `Abstractions/Time/IClock` + `SystemClock`, CQRS markers (`ICommand`, `ICommand<T>`, `IQuery<T>`), and the `AddApplicationBuildingBlocks()` DI extension. Must not depend on `BuildingBlocks.Infrastructure`/`BuildingBlocks.Web`, EF Core, or another bounded context — enforced by `DependencyRulesTests.BuildingBlocks_application_does_not_depend_on_infrastructure_or_web`.
- `BuildingBlocks.Contracts`: versioned integration-event envelopes/contracts shared across processes, including the common auditing contract (`AuditLogEvent`, `AuditChange`, `AuditActions`). It must not contain domain models.
- `BuildingBlocks.Infrastructure`: shared Kafka/outbox/auditing infrastructure referenced only by `Sales.Infrastructure` and `Inventory.Infrastructure` (never by Domain/Application) — `Outbox/OutboxMessage` (the unified outbox row, mapped independently by each service's own `outbox_messages` table configuration) + `Inbox/InboxMessage` (the unified inbox idempotency row, likewise mapped per-service; `Consumer` is a nullable superset column — Sales maps it `IsRequired()`, Inventory nullable) + `IOutboxPublisher`/`KafkaOutboxPublisher` (constructor-parameterized by producer name and `ActivitySource`), `Messaging/EventEnvelopeFactory`, EF Core audit components (`EfCoreAuditEntryFactory`, `AuditSaveChangesInterceptor`, aggregate resolver/enricher abstractions), and two purely-technical Kafka-consumer helpers, `Exceptions/PostgresExceptions.IsUniqueViolation` and `Tracing/KafkaConsumerActivity.Start`. Deliberately does **not** include the outbox-polling `BackgroundService` (`SalesOutboxPublisher`/`InventoryOutboxPublisher` stay separate, near-duplicate classes — considered too reliability-sensitive to merge) or a shared base class for the two Kafka consumer handlers (their dispatch logic is genuinely different business logic). See `docs/superpowers/specs/2026-07-09-shared-infrastructure-refactor-design.md` for the full rationale.
