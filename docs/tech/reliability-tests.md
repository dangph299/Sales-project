...
- **Transactional Outbox** — Integration event và audit event được ghi vào bảng `outbox_messages` trong **cùng transaction** với thay đổi nghiệp vụ. Nếu transaction commit, event chắc chắn tồn tại để publish; nếu rollback, cả hai cùng biến mất. Không có cửa sổ "đã đổi state nhưng mất event".
...
| AuditLog idempotency: cùng `AuditId` chỉ tạo một audit document | Automated (integration, Mongo) | `MongoReliabilityTests` |
| Audit auto outbox: entity added/modified/soft-deleted tạo audit event cùng transaction; rollback không để lại audit outbox | Automated (integration, SQLite) | `AuditOutboxTests` |
...
### Audit Playwright check

Kịch bản end-to-end cho audit product update:
...
Spec `audit-update.spec.ts` gọi Sales API, update product, rồi `AuditProbe` query MongoDB `audit.events` để kiểm tra document `ProductUpdated`.
