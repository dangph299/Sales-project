...
- MediatR pipeline của Sales có **2 behavior** phụ trách log: `LoggingBehavior` (Debug, theo dõi tiến trình) và `ErrorLoggingBehavior` (Warning/Error, nơi duy nhất log lỗi command/query) — mục 7.
...
Sales đăng ký 3 pipeline behavior theo thứ tự trong `Sales.Application/DependencyInjection.cs`:
...
`src/Services/Sales/Sales.Application/Services/Behaviors/LoggingBehavior.cs`:
...
`src/Services/Sales/Sales.Application/Services/Behaviors/ErrorLoggingBehavior.cs`:
...
Phân biệt 2 mức: **Warning** cho lỗi nghiệp vụ có thể lường trước (`ValidationException`, `DomainException`, `NotFoundException`, `ConflictException`, và `DbUpdateConcurrencyException` — nhận diện qua tên type vì Application không được reference EF Core), **Error** cho lỗi bất ngờ khác. `{@Request}` log nguyên object request (structured, destructure toàn bộ property) — hữu ích để biết chính xác input nào gây lỗi khi tra Seq.
