...
1. **`AuditLog.Worker` giờ log vào Seq.** `Program.cs` gọi `builder.Services.AddSerilog((_, loggerConfig) => loggerConfig.ConfigureSharedSinks(builder.Configuration, "audit-worker"))`, dùng chung `SerilogBootstrap` với Sales/Inventory và ghi Console + Seq + OTLP. Chi tiết: [Seqlog-usage-guide.md](Seqlog-usage-guide.md) mục 4.3.
